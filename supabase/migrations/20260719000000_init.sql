...
-- 2. Drop existing triggers/functions/tables if they exist to start fresh
...
BEGIN
    ...
    -- Rule: booking.check_in_date < p_check_out_date AND booking.check_out_date > p_check_in_date
    SELECT EXISTS (
        SELECT 1 
        FROM bookings 
...
-- Bookings Policy:
-- - Client CANNOT direct select all bookings
-- - Client CANNOT direct insert bookings (must use RPC function create_booking_secure)
-- - Client CANNOT direct update/delete bookings
-- - Admin can select and update bookings (uses service role which bypasses RLS on server side)
-- Wait, if public needs to look up their booking, they can run lookupBookingAction which uses service role or a specific secure function on server side.
-- For standard public, we do NOT define a select policy on bookings, keeping the table completely hidden from direct public queries.
-- However, we must allow the RPC function (which is SECURITY INVOKER by default, unless SECURITY DEFINER is set) to read and write.
-- Let's make the PostgreSQL function SECURITY DEFINER so that it can bypass RLS on behalf of clients, or we can configure policies.
-- Using SECURITY DEFINER on create_booking_secure allows the public to run it and perform inserts and overlap selects without direct table access.
